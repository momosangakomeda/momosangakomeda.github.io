...
//-------------------------------------------------------------------------------------------
kyureki = typeof exports === 'undefined' ?  {} : exports;

kyureki.sekki24 = new Array("春分","清明","穀雨","立夏","小満","芒種",
					"夏至","小暑","大暑","立秋","処暑","白露",
					"秋分","寒露","霜降","立冬","小雪","大雪",
					"冬至","小寒","大寒","立春","雨水","啓蟄");
kyureki.jukkan = new Array( "甲","乙","丙","丁","戊","己","庚","辛","壬","癸");
kyureki.junishi = new Array( "子","丑","寅","卯","辰","巳","午","未","申","酉","戌","亥","癸");
...
//　　戻り値　ユリウス日（世界時）
//-------------------------------------------------------------------------------------------
kyureki.getJulianDay = function(year, month, day, timeOffset) {
	var y,m,d;
	var a,b,c,jd;
	if(month>2) {
		y = year;
...
//-------------------------------------------------------------------------------------------
kyureki.getDateFromJulian = function(jd, timeOffset){
	var y,m,d;
    ...
    var datestr = kyureki.getStringDateISO(y, m, d, timeOffset);
...
//-------------------------------------------------------------------------------------------
kyureki.getStringDate = function(year,month,day,timeOffset) {
	var dateStr = "";
...
//-------------------------------------------------------------------------------------------
kyureki.getStringDateISO = function(year,month,day,timeOffset) {
	var dateStr = "";
	year = Math.floor(year);
	month = Math.floor(month);
	timeOffset = Math.floor(timeOffset);
	...
	dateStr += "-" + intToString(month,2);
	dateStr += "-" + intToString(d,2);
	dateStr += "T"+intToString(h,2);
	dateStr += ":" + intToString(m,2);
	dateStr += ":" + intToString(s,2);
    
	if(timeOffset<0) {
		dateStr += "-"+intToString(Math.abs(timeOffset),2);
	} else {
		dateStr += "+"+intToString(timeOffset,2);
	}
	dateStr += "00";
...
//-------------------------------------------------------------------------------------------
kyureki.getShortStringDate = function(year,month,day) {
	var dateStr = "";
...
function regLS(jd,ls) { 
	return regAngle180(LongitudeSun(kyureki.UTCtoTD(jd))-ls);
}
...
function get4Sekki(jd) {
	var timeOffset = -9;
	var k = Math.floor(LongitudeSun(kyureki.UTCtoTD(jd))/90);
	this.before = k*6;
...
function getMoonDay(jd) {
	...
	ls1 = Math.floor(LongitudeSun(kyureki.UTCtoTD(this.startDay))/30);	
	ls2 = Math.floor(LongitudeSun(kyureki.UTCtoTD(this.startDay+this.daysOfMonth))/30);	
	if(ls1==ls2) {
		this.chuki = -1;
	} else {
		this.chuki = ls2*2;
...
//-------------------------------------------------------------------------------------------
kyureki.getKyureki = function(year, month, day) {
	var timeOffset = -9;
	var sekki4, kmonth1, kmonth2, Kmonth;
	var jd0,jd;
	var k,k1,k2,diff;
	var jd0 = kyureki.getJulianDay(year, month, day, -9);  
    ...
    return new kDate(this.uruu, this.month, this.day, this.year, this.daysOfMonth);
};
...
//-------------------------------------------------------------------------------------------
function kDate(uruu, kmonth, kday, year, daysOfMonth) {
	this.year  = year;
	this.uruu  = uruu;
	this.month = kmonth;
	this.day   = kday;
